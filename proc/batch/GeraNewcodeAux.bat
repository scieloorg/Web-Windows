...
call batch\VerifExisteBase.bat %1\code\newcode

rem call batch\GeraInvertido.bat %1\code\newcode fst\newcode.fst %1\code\newcode
%CISIS_DIR%\mx %1\code\newcode btell=0 "pt_study_area" pft=@pft\sa.pft now > %2\sa_pt.seq
%CISIS_DIR%\mx seq=%2\sa_pt.seq create=%2\sa_pt -all now
%CISIS_DIR%\mx %2\sa_pt fst=@fst\sa.fst fullinv/ansi=%2\sa_pt -all now

%CISIS_DIR%\mx %1\code\newcode btell=0 "es_study_area" pft=@pft\sa.pft now > %2\sa_es.seq
